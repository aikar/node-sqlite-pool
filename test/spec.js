...
const Sqlite = require('../build/main');
...
it('Should allow named parameters to be used with prepared statements', (done) => {
  ...
  let p = db.use((conn) => {
    ...
    q = q.then(() => conn.exec('CREATE TABLE tbl (col TEXT)'));
    q = q.then(() => conn.prepare('INSERT INTO tbl(col) VALUES (:col)', { ':col': 'some text' })
      .then(stmt => stmt.run())
      .then((stmt) => {
        expect(stmt.lastID).to.equal(1);
    ...
    q = q.then(() => conn.prepare('INSERT INTO tbl(col) VALUES ($col)')
    ...
    q = q.then(() => conn.prepare('SELECT col FROM tbl WHERE 13 = @thirteen ORDER BY col DESC')
...
it('Should allow chaining Statement.run() calls', (done) => {
  ...
  let p = db.use((conn) => {
    let q = Promise.resolve();
    q = q.then(() => conn.exec('CREATE TABLE tbl (col1 TEXT, col2 TEXT, col3 TEXT)'));
